...
def refactorBefore(epic):
    ...
    epic = epic.replace('FITL', 'HTL')
    epic = epic.replace('F1TL', 'HTL')
    epic = epic.replace('FfTL', 'HTL')
    epic = epic.replace('H1iL', 'HTL')
    epic = epic.replace('1iTL', 'HTL')
    epic = epic.replace('IiTL', 'HTL')
    epic = epic.replace('ITEL', 'HTL')
    epic = epic.replace('1TEL', 'HTL')
    epic = epic.replace('F1F1_', 'HTL')
    epic = epic.replace('1_', 'L')
    epic = epic.replace('-', '')
    epic = epic.replace('-', '')
    epic = epic.replace('-', '')
...
def refactorEpic(epic):
    ...
    # print("before = ",epic)
    epic = epic.replace('7PG', 'ZPC') 
    epic = epic.replace('H1L', 'HTL') 
    epic = epic.replace('7PC', 'ZPC') 
    epic = epic.replace('TFIF', 'THF') 
    epic = epic.replace('1FIF', 'THF') 
    epic = epic.replace('TlIF', 'THF') 
    epic = epic.replace('T11F', 'THF') 
    epic = epic.replace('1FIF', 'THF') 
    epic = epic.replace('1F1F', 'THF') 
    epic = epic.replace('IFIF', 'THF') 

    
    
    epic = epic.replace('1YN', 'TYN') 
    epic = epic.replace('1UP', 'IUP')
    epic = epic.replace('iUP', 'IUP')
    epic = epic.replace('HP7', 'HPZ')
    epic = epic.replace('GP7', 'GPZ')
    epic = epic.replace('D1C', 'DTC')
    epic = epic.replace('D1G', 'DTC')
    epic = epic.replace('A7U', 'AZU')
    epic = epic.replace('B5N', 'BSN')
    epic = epic.replace('B3N', 'BSN')
    epic = epic.replace('K5X', 'KSX')
    epic = epic.replace('K3X', 'KSX')
    epic = epic.replace('7EB', 'ZEB')
    epic = epic.replace('FITL', 'HTL')
    epic = epic.replace('FfTL', 'HTL')
    epic = epic.replace('H1iL', 'HTL')
    epic = epic.replace('1iTL', 'HTL')
    ...
    if count_of_name == len(group_of_numbers):
        ...
        try:
            ...
            if temp.__len__() < count_of_name:
                for i in range(count_of_name-temp.__len__()):
                  temp.append('')
            if temp.__len__() == count_of_name:
                return temp
            else:
...
def main(pdf_file,destination):
    ...
    if REST_DETAILS_DICT['District'][1] == None or REST_DETAILS_DICT['District'][1] == '':
        print("front page error")
      
    if SIZE == 0:
        print(pdf_file)
        return
    # convert pdf name to csv name
    # csv_name = pdf_file[:len(pdf_file) - 4]
    # dictionary of lists
...
if __name__ == '__main__':
    ...
    for file in os.listdir(base):
        ...
        pdf.save(new_file)
        main(new_file, pdf_file[:pdf_file.find('.')]+".csv")
        os.remove(new_file)
